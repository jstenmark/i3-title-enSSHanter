...
class TestWindowTitleUpdater(unittest.TestCase):
    def setUp(self):
        ...
        self.title_updater.focused_window.name \
            .replace.return_value = "ssh ExampleServer"

    def test_set_window_title(self):
        self.title_updater.set_window_title("Test Title")
        self.title_updater.focused_window.command.assert_called_once_with(
            'title_format Test Title')

    def test_get_remote_title(self):
        self.args.font_color = "#ff0000"
        self.args.warn_text = "SSH Server"
        expected_title = '<span size="x-large" foreground="#ff0000">' + \
            ' SSH Server ssh ExampleServer</span>'
